...
def non_ref_based(im):
    ...
    """
    uiqm, uicm, uism, uiconm = getUIQM(np.array(Image.fromarray(np.uint8(im))))
    #uiqm, uicm, uism, uiconm = getUIQM(im)

    #uiqm = getUIQM(np.array(Image.fromarray(np.uint8(255*im)).resize((256, 256)))) #FuNie-Gan does this
    #uciqe = getUCIQE(im)
    uciqe = getUCIQE(np.array(Image.fromarray(np.uint8(im))))
    return uiqm, uicm, uism, uiconm, uciqe
